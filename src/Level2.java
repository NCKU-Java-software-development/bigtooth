...
public class Level2 extends JPanel implements ActionListener, KeyListener {
    Image level2bg=new ImageIcon("img/level2bg.jpg").getImage();
    GameFrame gamePanel;
    ...
    Timer ddcTimer;
    Timer movesliper;
    Timer ddcMoveTimer; // 新增的Timer
    int slipperxvel;
    int slipperyvel;
    int ToothVelocity = 2; // tooth velocity
    ImageIcon icon = new ImageIcon("img/bigtooth.png");
    ImageIcon ddc = new ImageIcon("img/ddc.jpg");
    ImageIcon boom = new ImageIcon("img/boom.png");
    final int BigtoothWidth = icon.getIconWidth();
    final int BigtoothHeight = icon.getIconHeight();
    int toothSpeed = 500;
    int attack = 10;
    Point bigtoohPoint;
    Point prevpt;
    ArrayList<toothgroup> toothGroups = new ArrayList<>();
    ArrayList<slippergroup> slippers = new ArrayList<>();
    ...
    String BackgroundMusic = "/background_music.mp3";
    SoundPlayer soundPlayer = new SoundPlayer(HitSound);
    
    Level2(GameFrame gamepanel) {
        ...
        ddcTimer = new Timer(200, new ActionListener() { // collide on bigtooth ddc attack frequency
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                slippergroup sl = new slippergroup((int) (ddcX + 30), (int) (ddcY + 30), slipperxvel, slipperyvel);
                slippers.add(sl);
                repaint();
            }
        });
        ...
        movesliper = new Timer(10, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                Iterator<slippergroup> iterate = slippers.iterator();
                while (iterate.hasNext()) {
                    slippergroup sg = iterate.next();
                    sg.update();
        ...
        // 新增DDC自動水平移動的Timer
        ddcMoveTimer = new Timer(10, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                // 更新DDC位置
                ddcX += ddcXVel;
                // 檢查邊界，讓DDC在水平邊界間來回移動
                if (ddcX < 0 || ddcX + ddcWidth > screenSize.width) {
                    ddcXVel = -ddcXVel;
                }
                repaint();
            }
    ...
    public boolean CheckCollisionDDC(toothgroup tg) {
        Rectangle toothRect = new Rectangle(tg.x, tg.y, tooth[ToothIndex].getWidth(null), tooth[ToothIndex].getHeight(null));
        Rectangle ddcRect = new Rectangle(ddcX, ddcY - 50, ddcWidth, ddcHeight);
        return toothRect.intersects(ddcRect);
    }

    public boolean CheckCollisionBigtooth(slippergroup sl) {
    ...
    public class slippergroup {
        ...
        slippergroup(int x, int y, int xVel, int yVel) {
            this.x = x;
            this.y = y;
            this.yvel = yVel;
            this.xvel = xVel;
    ...
    public void paintComponent(Graphics g) {
        ...
        icon.paintIcon(this, g, (int) bigtoohPoint.getX(), (int) bigtoohPoint.getY()); // bigtooth
        ddc.paintIcon(this, g, ddcX, ddcY);
        // 绘制所有 toothgroup 对象
        for (toothgroup tg : toothGroups) {
            tg.paint(g);
        }
        for (slippergroup sl : slippers) {
            sl.paint(g);
        }
